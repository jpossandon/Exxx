...
% hand in two different lateral position, meanwhile gaze can be positioned
% in three location. This results 6 conditions of tactile/gaze position:
%
%   Gaze:        2     1     3
%   Touch:          a     b
%
%   a,b correspond to different external positions
%   in (1,b & 2a) and (1,a & 3,b) tactile stimulus is in same location with respect to the gaze
%   
% Subjects task is to decide whether a transient visual stimulus after the
% tactile stimus was presented to the right or left of it.
% Response with mouse
%
% Temporal profile
%
%     as--------x--------ts-----x---------vs----response
% ms:    500    rnd(500)   500  rand(500)   subject
%
%     as: audio signal trial start
%     ts: tactile stimulus (50 ms-200 Hz)
%     vs: dot ( visual degress, 50 ms)
%     response: mouse, left-right button
%
%
...
win.fixpos                  = [win.res(1)/2, win.res(2)/2;...
                                win.res(1)/6, win.res(2)/2;...
                                5*win.res(1)/6, win.res(2)/2];
...
win.stim_dur                = .05;                                         % duration of tactile stimulation. The vibrator takes some time to stop its motion so for around 50 ms we use 25 ms of stimulation time (ask Tobias for the exact latencies they have measured)
win.tact_fixlat             = .75;
win.tact_rndlat             = .25;
win.tact_visfix             = .75;
win.tact_visrnd             = .25;
