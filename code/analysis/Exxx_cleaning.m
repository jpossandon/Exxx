...
if ismac    
    cfg             = eeg_etParams_Exxx('sujid',suj,'expfolder','/Users/jossando/trabajo/Exxx/'); % this is just to being able to do analysis at work and with my laptop
else
    cfg             = eeg_etParams_Exxx('sujid',suj);
end
...
% cfg                                 = eeg_etParams_Exxx(cfg,'clean_bad_channel_criteria',.25);
% check_session(cfg)
% 
% % re-check bad segments now whitout taking in accound bad channels (done in badsegments and check_session)
%  cfg                                = eeg_etParams_Exxx(cfg,'clean_movwin_length',.256,'clean_mov_step',.006);
...
% % bad because of trend, longer
 cfg                                = eeg_etParams_Exxx(cfg,'clean_movwin_length',1,...
                                          'clean_mov_step',.06);
...
cfg                                 = eeg_etParams_Exxx(cfg,'clean_bad_channel_criteria',.20);
check_session(cfg)
...
cfg                                = eeg_etParams_Exxx(cfg,'clean_movwin_length',1,'clean_mov_step',.06);
